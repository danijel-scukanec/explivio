using Explivio.AIWorker.Consuming;
...
if (!string.IsNullOrWhiteSpace(sqlConnectionString))
{
    builder.Services.AddDbContext<WorkerDbContext>(options =>
        options.UseSqlServer(sqlConnectionString, sql =>
            sql.MigrationsHistoryTable("__EFMigrationsHistory", WorkerDbContext.Schema)));
}
...
// Apply the worker's own inbox migration on startup. It targets only the "worker" schema, so it
// never touches the API's tables. Retried briefly to tolerate SQL still warming up under the AppHost.
if (!string.IsNullOrWhiteSpace(sqlConnectionString))
{
